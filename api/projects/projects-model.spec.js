...
describe('projects-model', () => {
  ...
  describe('findDeadlinesById(id)', () => {
    it('should return an array of deadlines associated with the given (project) id', async () => {
      const deadlines = await Projects.findDeadlinesById(1);
      expect(Array.isArray(deadlines)).toBe(true);
      const testDeadline1 = {deadline_type: 'testType1', deadline: JSON.stringify(new Date('2030-01-01'))};
      const testDeadline2 = {deadline_type: 'testType2', deadline: JSON.stringify(new Date('2020-01-02'))};
      expect(deadlines[0]).toEqual(testDeadline1);
      expect(deadlines[1]).toEqual(testDeadline2);
